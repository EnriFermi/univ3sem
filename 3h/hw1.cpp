...
class list{
    ...
    public:
        ...
        bank_account front() const;
        bank_account back() const;
        void pop_front();
        void pop_back();
...
bank_account list::front() const{
    return this->first->elem;
}
bank_account list::back() const{
...
class queue : private list{
        ...
        static int dq_msize;
    public:
        queue(int size=dq_msize): q_base(list()), q_msize(size){};
        void back(const bank_account & elem);
        bank_account front() const;
        void pop();
        bool empty();
        int size();
        bool full();
        void print();
        void set_max_size(int m_size){
            dq_msize = m_size;
        }
};
int queue::dq_msize = 2;

void queue::back(const bank_account& elem){
    if(q_base.size() < q_msize){
        q_base.push_back(elem);
    } else {
        cout << "Queue overflow"<< endl;
    }
}
bank_account queue::front() const{
    return q_base.front();
}
void queue::pop(){
    return q_base.pop_front();
}
bool queue::empty(){
    return q_base.empty();
}
